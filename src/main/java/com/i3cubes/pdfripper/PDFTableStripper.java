...
public class PDFTableStripper extends PDFTextStripper
{
	 ...
	 */
	public void readTable(String file) throws IOException
	{
		...
		try (PDDocument document = PDDocument.load(new File(file)))
		{
			...
			stripper.setRegion(new Rectangle((int) Math.round(0.3*res), (int) Math.round(4*res), (int) Math.round(7.7*res), (int) Math.round(9.0*res)));
